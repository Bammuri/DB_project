...
<body>
  ...
  <fieldset>
  <h2> 검색하기 </h2>
  <form name="search.php" method = "post" action = "market_search.php">
          ...
          while($row = mysql_fetch_row($result)) {
            ...
            for($i=0;$i<$fields-1;$i++) {
             ...
             if($i==$fields-2){echo("<td>
               <form name='위치보기' method ='post' action = 'market_map.php'>
               <input type = 'hidden' value = 'echo $id_tmp' name = 'id_t'>
               <input type = 'hidden' value = 'echo $pw_tmp' name = 'pw_t'>
               <input type='text' value='$row[12]' name='la'>
               <input type='text' value='$row[13]' name='al'>
             <input type='submit' value='위치보기' ></td>");
              }
              else{
              echo("<td> $row[$i] </td>");}
            }
