...
<body>
  <form name="search_for_delete.php" method = "post" action = "main.php">
    <input type = "hidden" value = "<?echo $_POST['id_t'];?>" name = "id">
    <input type = "hidden" value = "<?echo $_POST['pw_t'];?>" name = "password">
  ...
  <tbody>
    <?
      ...
      while($row = mysql_fetch_row($result)) {
        ...
        echo("<td>
        <form method = 'post' action = 'market_delete.php'>
        <input type= 'hidden' name='no' value='$row[0]' >
        <input type= 'hidden' name='name' value='$row[1]' >

        <input type = 'hidden' value = '$id_t' name = 'id_t'>
        <input type = 'hidden' value = '$pw_t' name = 'pw_t'>
