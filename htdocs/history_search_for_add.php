      ...
      $sql = "SELECT * from market where $where order by no";
          ...
          while($row = mysql_fetch_row($result)) {
            if(!empty($_POST['isSearch'])) {
              mysql_query($sql_2,$connect);
            }
            echo("<tr>");

            for($i=0;$i<$fields-1;$i++) {

             if($i==$fields-2){echo("<td>
               <form name='북마크 추가하기' method ='post' action = 'history_add.php'>
               <input type = 'hidden' value = '$id_use' name = 'id_t'>
               <input type = 'hidden' value = '$pw_use' name = 'pw_t'>
               <input type = 'text' value ='$id_use' name = 'id_t'>
               <input type='text' value='$row[0]' name='no'>
               <input type='text' value='$row[1]' name='name'>
               <input type='text' value='$row[3]' name='bigname'>
               <input type='text' value='$row[5]' name='midname'>
               <input type='text' value='$row[7]' name='stdname'>
             <input type='submit' value='북마크 추가하기' ></td>");
              }
              else{
              echo("<td> $row[$i] </td>");}
            }
